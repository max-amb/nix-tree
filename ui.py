...
from textual.widgets import Input, Label, ListView, ListItem, OptionList, Tree, Header, Footer, TabbedContent, \
    TabPane, Button, RadioSet, _tree

type VariableNode = _tree.TreeNode[dict]
type ConnectorNode = _tree.TreeNode
...
class ModifyScreen(ModalScreen[str]):
    ...
    def on_input_submitted(self, new_data: Input.Submitted) -> None:
        ...
        Note:
            It also performs a small bit of input validation, if a list hasn't got both brackets or a string doesn't
            have a speech mark, it does not modify the variable
        ...
        if self.__value != new_data.value:
            if self.__type == Types.LIST and ("[" not in new_data.value or "]" not in new_data.value):
                self.notify("You lost a bracket! Not updating")
                self.app.pop_screen()
            elif self.__type == Types.STRING and ("'" not in new_data.value):
...
class OptionsScreen(ModalScreen[str]):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(classes="modifytext"):
            ...
            with Center():
                yield Label(f"Current value: {self.__value}", classes="box")
...
def work_out_full_path(current_node: ConnectorNode, path: list) -> list:
    """A function when given a node, works out its full path using parent relations

    Args:
        current_node: ConnectorNode - the current node we are checking
...
class AddScreenPath(ModalScreen[list]):
    ...
    def recursive_addition(self, node: ConnectorNode, path: list, data: str, path_as_list: list,
        ...
        Args:
            node: ConnectorNode - the current node we are analysing (as the function is recursive this changes in every
            stack frame)
...
class SectionOptionsScreen(ModalScreen[list[str]]):
    ...
    def recursive_deletion(self, node: ConnectorNode) -> None:
        """Recursively deletes a section by visiting children and then deleting all the data there and so on

        Args:
            node: ConnectorNode - the current node being checked
...
class UI(App):
    ...
    def setup_tree(self) -> Tree:
        tree: Tree[dict] = Tree("configuration.nix", id="tree")
        tree.root.expand()
        characters = tree.root.add("grub", expand=True)
        characters.add_leaf("listie=['a', 'b', 'c']", data={"grub.listie": "['a', 'b', 'c']", "type": Types.LIST})
        x = characters.add("FISH")
        x.add_leaf("a=true", data={"grub.FISH.x": "true", "type": Types.BOOL})
        characters.add_leaf("enable=true", data={"grub.enable": "true", "type": Types.BOOL})
        characters.add_leaf("useOsProber=false", data={"grub.useOsProber": "false", "type": Types.BOOL})
        characters.add_leaf("name=max", data={"grub.name": "max", "type": Types.UNIQUE})
        characters.add_leaf("num=156", data={"grub.num": "156", "type": Types.INT})
        return tree
            ...
            if action:
                ...
                match action.split(" ")[0]:
                    ...
                    case "Added":
                        ...
                        node_to_delete: VariableNode = self.recursive_searching_for_var(
                            self.query_one(Tree).root,
                            path.split("."),
                            variable
                        )
                    ...
                    case "Change":
                        ...
                        variable_to_change: VariableNode = self.recursive_searching_for_var(
                            self.query_one(Tree).root,
                            post[0].split("."),
                            post[1]
                        )
                    ...
                    case "Section":
                        match action.split(" ")[-1]:
                            case "deleted":
                                ...
                                section_node: ConnectorNode = self.recursive_searching_for_connector(
                                    self.query_one(Tree).root,
                                    path_before_section
                                )
    ...
    def recursive_addition(self, node: ConnectorNode, path: list, data: str, data_type: Types, full_path: str) -> None:
    ...
    def recursive_searching_for_var(self, node: ConnectorNode, path: list, variable: str) -> VariableNode:
        """Recursively searches for a var so the undo function can do something with it - often deletion

        Args:
            node: ConnectorNode - the starting node, often the root node
        ...
        Returns:
            VariableNode - the found variable node
        """
    ...
    def recursive_searching_for_connector(self, node: ConnectorNode, path: list) -> ConnectorNode:
        """Recursively searches for a section so the undo function can add and delete sections

        Args:
            node: ConnectorNode - the starting node
            path: list - a path to store how close we are and when to stop

        Returns:
            ConnectorNode - the found connector node
        """
        if len(path) > 1:
            for child in node.children:
                if child.label.plain == path[0]:
